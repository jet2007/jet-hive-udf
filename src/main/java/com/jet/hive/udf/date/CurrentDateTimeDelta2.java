 ...
 * 当前时间，进行日期时间计算,返回yyyyMMddHHmmss格式
 */


public class CurrentDateTimeDelta2 extends UDF {
	private Text result = new Text();
	
	/**
	 * @param args 参数1：offsets；返回yyyyMMddHHmmss格式
	 * @return
	 */
	public Text evaluate(Text... args) {
		...
		String re = DateUtils.getFormatDate(reDate,"yyyyMMddHHmmss");
		result.set(re);
		return result;
