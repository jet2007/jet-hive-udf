 ...
 * 当前时间，进行日期时间计算,返回yyyy-MM-dd格式
 */


public class CurrentDateDelta extends UDF {
	private Text result = new Text();
	
	/**
	 * @param args 参数1：offsets；返回yyyy-MM-dd格式
	 * @return
	 */
	public Text evaluate(Text... args) {
		...
		String re = DateUtils.getFormatDate(reDate,"yyyy-MM-dd");
		result.set(re);
		return result;
	}
}
